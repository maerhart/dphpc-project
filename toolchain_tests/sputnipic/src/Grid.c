...
void setGrid(struct parameters *param, struct grid *grd) {
  ...
  grd->XN = (FPfield***) ptrArr((void**) &grd->XN_flat, sizeof(FPfield), 3, grd->nxn, grd->nyn, grd->nzn);
  grd->YN = (FPfield***) ptrArr((void**) &grd->YN_flat, sizeof(FPfield), 3, grd->nxn, grd->nyn, grd->nzn);
  grd->ZN = (FPfield***) ptrArr((void**) &grd->ZN_flat, sizeof(FPfield), 3, grd->nxn, grd->nyn, grd->nzn);
...
void printGrid(struct grid *grd) {
  printf("\n**** Grid Information - Include Ghost Cells ****\n");
  printf("Number of cell: -X = %d, -Y = %d, -Z = %d\n", grd->nxc, grd->nyc, grd->nzc);
  printf("Lx = %f, Ly = %f, Lz = %f\n", grd->Lx, grd->Ly, grd->Lz);
  printf("PERIODICX = %d, PERIODICY = %d, PERIODICZ = %d\n\n", grd->PERIODICX, grd->PERIODICY, grd->PERIODICZ);
}
