...
void initGEM(struct parameters *param, struct grid *grd, struct EMfield *field,
  ...
  // print settings
  printf("*************************************************\n");
  printf("**  Initialize GEM Challenge with Pertubation  **\n");
  printf("*************************************************\n");
  printf("** B0x = %f\n", param->B0x);
  printf("** B0y = %f\n", param->B0y);
  printf("** B0z = %f\n", param->B0z);
  printf("** Delta (current sheet thickness) = %f\n", param->delta);
  for (int is = 0; is < param->ns; is++) {
    printf("** rho species %d = %f", is, param->rhoINIT[is]);
    if (is < 2)
  ...
  if (param->RESTART) {
    ...
    printf("Restart not available in C version, sorry...\n");
...
#pragma omp parallel for private(xpert,ypert,exp_pert)
            ...
            for (int jj = 0; jj < part[is].npcely; jj++)
              for (int kk = 0; kk < part[is].npcely; kk++) {
                ...
                if (part[is].v[counter] > param->c) {
                  printf("ERROR - v VELOCITY > c !");
                  exit(EXIT_FAILURE);
                }
                if (part[is].w[counter] > param->c) {
                  printf("ERROR - w VELOCITY > c !");
                  exit(EXIT_FAILURE);
                }
  ...
  //   cout << "Restart not available in C version, sorry..." << endl;
  //   //save_ic_data(field, field_aux, grd, ids, part, param);
  // }
  // else {
  //   cout << "folder " << param->RestartDirName << " does not exist, not recording IC." << endl;
  // }
...
// perturbation localized in X
  ...
  // print settings
  printf("*************************************************\n");
  printf("**  Initialize UNIFORM Plasma Distribution     **\n");
  printf("*************************************************\n");
  printf("** B0x = %f\n", param->B0x);
  printf("** B0y = %f\n", param->B0y);
  printf("** B0z = %f\n", param->B0z);
