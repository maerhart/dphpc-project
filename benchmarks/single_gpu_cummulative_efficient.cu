...
const int stages = 2;
const unsigned long long hostBufSize = 1ull << 27;
...
struct SharedState {
    SharedState() {
        ...
        CUDA_CHECK(cudaMallocHost(&hostBuf, hostBufSize));
    }
...
//    }
    ...
    void asyncTransfer(int dst) {
        ...
        for (size_t s = 0; s < dsr.transferSize; s += hostBufSize) {
            size_t chunk = hostBufSize;
            if (s + chunk > dsr.transferSize) chunk = dsr.transferSize - s;
            CUDA_CHECK(cudaMemcpyAsync(hostBuf, ((char*)dsr.sendPtr) + s, chunk, cudaMemcpyDefault, hostCopyStream));
            CUDA_CHECK(cudaMemcpyAsync(((char*)dsr.recvPtr) + s, hostBuf, chunk, cudaMemcpyDefault, hostCopyStream));
        }
        CUDA_CHECK(cudaStreamSynchronize(hostCopyStream));
...
__global__ void asyncTransferKernel(SharedState* ss) {
    ...
    for (int m = 0; m < numMeasurements; m++) {
        ...
        for (int stage = 0; stage < stages; stage++) {
            ...
            if (ss->rank() == 0) {
                //printf("t1 %lld t2 %lld t2-t1 %lld ", t1, t2, t2-t1);
